...
type CustomMultiSelectBeforeExpiryProps<T extends FieldValues> = {
  control: Control<T>;
  name: Path<T>;
  options?: Option[];
  placeholder?: string;
...
export default function CustomMultiSelectBeforeExpiry<T extends FieldValues>({
  control,
  name,
  options = [],
  placeholder = "Select...",
  isDisabled = false,
}: CustomMultiSelectBeforeExpiryProps<T>) {
  ...
  const CustomMenuList = (props: MenuListProps<Option, true>) => {
    ...
    const addOption = () => {
      ...
      const newOption = {
        value: `${inputDays} Days before expiry`,
        label: `${inputDays} Days before Expiry`,
      };
