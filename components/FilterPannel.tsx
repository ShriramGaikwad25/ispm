...
const FilterPanel: React.FC<FilterPanelProps> = ({ data, selected, onSelect, onClear }) => {
  return (
    <>
      {Object.entries(data).map(([category, items]) => (
        <div key={category}>
          <div className="flex justify-between items-center mb-2 border-gray-300 pb-4 p-2">
            <h3 className="text-lg text-gray-700">{category.replace(/([A-Z])/g, " $1")}</h3>
            <button
          ...
          <div className="space-y-2 pl-2 pr-2">
            {items.map((item, index) => (
              ...
              >
                ...
                <span className={`font-semibold absolute -right-2 bg-white border p-1 text-[12px]  rounded-sm ${
                    selected[category] === index
