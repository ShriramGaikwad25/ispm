...
import { CheckCircleIcon, Flag, User, ChevronRight, ChevronUp, ChevronDown } from "lucide-react";
...
const TreeClient: React.FC<TreeClientProps> = ({
  ...
  useEffect(() => {
    if (users.length > 0) {
      ...
      const isSelectedUserInCurrentPage = selectedUser && users.some(user => user.id === selectedUser.id);
    ...
    try {
      ...
      const entitlementPromises = accounts.map(async (account: any) => {
        ...
        return entitlements.map((item: any, index: number) => ({
          ...
          entitlementDescription: item.entitlementInfo?.entitlementDescription ?? "",
  ...
  const entitlementsColumnDefs = useMemo<ColDef[]>(
    () => [
        ...
        cellRenderer: (params: ICellRendererParams) => {
          ...
          const riskAbbr = itemRisk === "High" ? "H" : itemRisk === "Medium" ? "M" : "L";
          const riskColor = itemRisk === "High" ? "red" : itemRisk === "Medium" ? "orange" : "green";
                return (
                ...
                <div className="text-xs text-gray-600 break-words leading-relaxed entitlement-description w-full" title={entitlementDescription}>
                  {entitlementDescription}
                </div>
              ) : (
          ...
          const riskColor = appRisk === "High" ? "red" : appRisk === "Medium" ? "orange" : "green";
              return (
                <div className="flex items-center gap-2">
    ...
    <div className="flex h-[calc(100vh-120px)] relative">
      ...
      <div 
        className={`absolute left-0 top-0 bottom-0 bg-white border-r border-gray-200 flex flex-col transition-all duration-300 ease-in-out z-30 shadow-lg ${
          isSidebarHovered ? 'w-64 translate-x-0 pr-0' : 'w-12 -translate-x-0 pr-2'
        }`}
      ...
      >
        ...
        <div className={`flex-1 flex flex-col justify-start pt-0 pb-1 ${
          ...
          {sidebarLoading ? (
            ...
            users
              .filter((user) => (user.fullName || "").toLowerCase().includes(userSearch.trim().toLowerCase()))
              .map((user, index) => (
            <div
              key={user.id}
            ...
            >
              <div className={`flex items-center ${isSidebarHovered ? 'gap-3 justify-start' : 'justify-center'}`}>
                {/* User Avatar/Initials */}
                <div className={`relative flex-shrink-0 ${!isSidebarHovered ? 'mx-auto' : ''}` }>
                  {!isSidebarHovered ? (
                    <div className={`w-7 h-7 rounded-full bg-blue-600 text-white flex items-center justify-center font-semibold text-xs ${
                      selectedUser?.id === user.id ? 'bg-blue-700 ring-2 ring-blue-300' : ''
                    }`}>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      <span className={`font-medium text-sm ${
                        selectedUser?.id === user.id ? 'text-blue-800 font-semibold' : 'text-gray-900'
                      }`}>
                        {user.fullName}
                      </span>
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-shrink-0">
                    <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-full flex items-center justify-center text-white font-semibold text-lg">
                      {selectedUser.fullName ? selectedUser.fullName.split(' ').map(n => n[0]).join('').toUpperCase() : '??'}
                    </div>
                  </div>
                      ...
                      <div className="space-y-2">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-1 sm:space-y-0">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-sm text-gray-900 font-medium whitespace-nowrap truncate">
                          ...
                          {filterOptions.map((filter) => {
                            ...
                            return (
                              <div
                                ...
                                className={`
                                  px-2 py-1 rounded-md border cursor-pointer transition-all duration-200 hover:shadow-sm text-[10px]
                                  ${isSelected 
                                    ? `${filter.color} shadow-sm transform scale-105` 
                  ...
                  {/* Right Column: Status and Action Buttons */}
                  <div className="flex-shrink-0 w-full sm:w-auto sm:min-w-[240px]">
                    <div className="space-y-2">
                      {/* Status */}
                      <div className="flex items-center space-x-2">
                      ...
                      <div className="flex items-center space-x-2.5">
                        <button 
                          className="flex items-center px-2.5 py-1.5 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors duration-200 text-xs font-medium"
                        ...
                        <button 
                          className="flex items-center space-x-2 px-2.5 py-1.5 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-md hover:from-purple-700 hover:to-pink-700 transition-all duration-200 text-xs font-medium"
        ...
        <SelectAll
          ...
          <input
            ...
            onChange={(e) => {
                    if (entitlementsGridApiRef.current) {
                      entitlementsGridApiRef.current.setGridOption('quickFilterText', e.target.value);
              }
            }}
          />
              </div>
          ...
          <AgGridReact
            ...
            className="ag-main"
                ...
                <div className="flex items-center justify-center h-64">
                  <div className="text-gray-500">No entitlements found for this user.</div>
                </div>
              )}
            </div>
          </>
