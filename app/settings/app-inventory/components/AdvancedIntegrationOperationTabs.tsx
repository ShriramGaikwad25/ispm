...
export default function AdvancedIntegrationOperationTabs({
  ...
  return (
    <div className={className}>
      <div className="grid grid-cols-8 w-full gap-0.5 mb-3 border-b border-gray-200 pb-2">
        {ADVANCED_INTEGRATION_OPERATION_TABS.map((tab, index) => (
          <button
            key={tab.id}
            type="button"
            onClick={() => setActiveIndex(index)}
            className={`min-w-0 rounded px-1 py-1 text-[11px] leading-tight font-medium transition-colors text-center ${
              activeIndex === index
