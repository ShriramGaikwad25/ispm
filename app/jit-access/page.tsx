...
import UserGroupTab from "@/app/access-request/UserGroupTab";
import ReviewTab from "@/app/access-request/ReviewTab";
import JitSelectAccessTab from "./JitSelectAccessTab";
import JitDetailsTab from "./JitDetailsTab";
...
function resolveEntitlementNameForCatalogId(
  catalogId: string,
  accessItems: JitAccessRequestPayload["accessItems"]
): string {
...
const JitAccess: React.FC = () => {
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      ...
      const isLeavingJitAccess = isInternal && !href.startsWith("/jit-access");
      if (!isLeavingJitAccess) return;
      const hasSelections =
  ...
  useEffect(() => {
    return () => {
      clearJitAccessSelections(clearCart, clearUsers, clearItemDetails);
    };
  ...
  const apiRoles = React.useMemo(() => {
    ...
    if (isKfBrRoleRows) {
      return catalogData.map((row, idx) => {
        const raw = row.role_name !== undefined && row.role_name !== null ? String(row.role_name) : "";
        const name = raw.trim() || "Unnamed access";
        return { id: name || String(idx), name, risk: "Low" as const, description: "", type: "role" as const, catalogRow: { ...row, type: "role" } };
      });
    }
    ...
    const nameKey = findKey([(k) => k === "name", (k) => k === "entitlementname", (k) => k === "entitlement_name", (k) => k === "applicationname"]) || keys[0];
    const riskKey = findKey([(k) => k === "risk", (k) => k.endsWith("_risk"), (k) => k.includes("risk")]) || keys[1] || keys[0];
    const descriptionKey = findKey([(k) => k === "description", (k) => k === "entitlementdescription", (k) => k === "entitlement_description", (k) => k === "business_objective"]) || keys[2] || keys[0];
    ...
    return catalogData.map((row, idx) => {
      const rawName = row[nameKey] !== undefined && row[nameKey] !== null ? String(row[nameKey]) : "";
      const rawRisk = row[riskKey] !== undefined && row[riskKey] !== null ? String(row[riskKey]) : "";
      const rawDesc = row[descriptionKey] !== undefined && row[descriptionKey] !== null ? String(row[descriptionKey]) : "";
      const rawType = typeof row.type === "string" ? row.type : typeof row.Type === "string" ? row.Type : undefined;
      const idValue = (idKey && row[idKey]) || row.catalogid || row.entitlementid || row.appinstanceid || idx;
      return { id: String(idValue).trim(), name: rawName || "Unnamed access", risk: normalizeRisk(rawRisk), description: rawDesc, type: rawType, catalogRow: row };
    });
  }, [catalogData]);
  ...
  const buildPayload = (): JitAccessRequestPayload => {
  ...
  const handleSubmit = async () => {
    ...
    const selfApiUserId = payload.requestFor === "self" ? await resolveCurrentApiUserId() : "";
    ...
    try {
      ...
      const failed = !res.ok || (responseBody != null && isSubmitBusinessFailure(responseBody));
      ...
      if (failed) {
        ...
        const dbErr = dbResp && typeof dbResp === "object" && typeof (dbResp as Record<string, unknown>).error === "string"
        ...
        if (isTrainingFailure) {
          ...
          setSubmitFailureDialog({
            ...
            message: errMsg || dbErr || (!res.ok ? `HTTP ${res.status}` : "") || (responseText ? responseText.slice(0, 500) : "") || "Unknown error",
          });
        }
        if (!res.ok) console.error("Submit request failed:", res.status, responseText);
        else console.warn("Submit request business failure:", responseBody);
        return;
      }

      console.log("JIT submit succeeded:", responseBody ?? responseText);
    } catch (err) {
    ...
    fetch(url, {
      ...
      .then((data) => {
        ...
        rows.forEach((r: any) => {
          ...
          if (id && !seen.has(id)) { seen.add(id); list.push({ id, name: name || id }); }
        });
  ...
  React.useEffect(() => {
    ...
    const body =
      showApplicationInstancesOnly
        ? { query: "SELECT * FROM vw_catalog WHERE type = 'ApplicationInstance' ORDER BY appinstanceid", parameters: [] }
        : catalogTypeFilter === "Roles"
          ? { query: "select role_name from kf_br_role_t", parameters: [] }
          : catalogTypeFilter === "Tags"
            ? { query: trimmedTag ? `SELECT * FROM vw_catalog WHERE type = 'Entitlement' AND tags ILIKE '%${trimmedTag}%' ORDER BY appinstanceid` : "SELECT * FROM vw_catalog WHERE type = 'Entitlement' ORDER BY appinstanceid", parameters: [] }
    ...
    fetch("https://preview.keyforge.ai/entities/api/v1/ACMECOM/executeQuery", {
      ...
      .finally(() => {
        if (catalogFetchKeyRef.current === fetchKey) { setCatalogLoading(false); catalogFetchKeyRef.current = null; }
      });
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      if (!raw) { setSelectedGroups([]); return; }
      const parsed = JSON.parse(raw);
      if (Array.isArray(parsed)) {
        setSelectedGroups(parsed.map((g) => ({ value: String(g?.value ?? "").trim(), label: String(g?.label ?? "").trim() })).filter((g) => g.value && g.label));
      } else { setSelectedGroups([]); }
    } catch { setSelectedGroups([]); }
  }, [currentStep]);

  const userTabs = [
    { label: "User Search", component: UserSearchTab },
    { label: "User Group", component: UserGroupTab },
  ];
  ...
  return (
    <div className="pt-16">
        ...
        <>
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 px-6 py-3 mb-6">
            <div className="flex justify-center items-center gap-4">
              <span
                className={`text-sm font-medium cursor-pointer ${selectedOption === "self" ? "text-blue-600 font-semibold" : "text-gray-600"}`}
              ...
              <span
                className={`text-sm font-medium cursor-pointer ${selectedOption === "others" ? "text-blue-600 font-semibold" : "text-gray-600"}`}
          ...
          {selectedOption === "others" && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
              <HorizontalTabs tabs={userTabs} activeIndex={activeTab} onChange={setActiveTab} />
            </div>
          )}
        </>
      )}
        ...
        >
          <div className="flex max-h-[85vh] w-full max-w-lg flex-col overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/5">
            ...
            <div className="flex flex-wrap justify-center gap-2 border-t border-gray-200 bg-white px-5 py-3">
              <button
                type="button"
                onClick={() => { setShowSubmitSuccessPopup(false); router.push("/"); }}
              ...
              <button
                type="button"
                onClick={() => { setShowSubmitSuccessPopup(false); router.push("/jit-access"); }}
                className="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100"
              >
                JIT Access
              </button>
              <button
                type="button"
                onClick={() => { setShowSubmitSuccessPopup(false); router.push("/track-request"); }}
            ...
            {submitFailureDialog.variant === "training" ? (
              <>
                <div className="flex gap-3 border-b border-amber-200/80 bg-gradient-to-br from-amber-50 via-white to-white px-5 py-4">
                  ...
                  <div className="min-w-0 pt-0.5">
                    <h2 id="submit-failure-title" className="text-lg font-semibold tracking-tight text-gray-900">Request Failed...</h2>
                    <p className="mt-1 font-mono text-[11px] leading-snug text-amber-900/70">{submitFailureDialog.code}</p>
                  </div>
                </div>
                  ...
                  <div className="flex-1 overflow-y-auto px-5 pb-1">
                    <ul className="space-y-3">
                      {submitFailureDialog.lines.map((line, idx) => (
                        <li key={`${line.entName}-${idx}`} className="rounded-xl border border-gray-200 bg-white p-3.5 shadow-sm">
                          <p className="text-[15px] font-semibold leading-snug text-gray-900">{formatEntitlementDisplayName(line.entName)}</p>
                          <div className="mt-2 flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                            <span className="text-xs font-medium uppercase tracking-wide text-gray-500">Training code</span>
                            <span className="font-mono text-sm font-medium text-gray-900">{line.trainingCode?.trim() ? line.trainingCode : "Not specified"}</span>
                          </div>
                          <p className="mt-2 border-t border-gray-200/90 pt-2 text-sm leading-relaxed text-gray-700">{line.message}</p>
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                <div className="flex justify-end border-t border-gray-200 bg-white px-5 py-3">
                  <button type="button" onClick={handleCloseSubmitFailureDialog} className="rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700">Close</button>
                </div>
              </>
            ) : (
              <>
                <div className="flex gap-3 border-b border-gray-200 bg-gray-50 px-5 py-4">
                  ...
                  <div className="min-w-0 pt-0.5">
                    <h2 id="submit-failure-title" className="text-lg font-semibold text-gray-900">{submitFailureDialog.title}</h2>
                  </div>
                </div>
                <div className="max-h-[min(50vh,320px)] overflow-y-auto bg-gray-50 px-5 py-4">
                  <p className="whitespace-pre-wrap text-sm leading-relaxed text-gray-700">{submitFailureDialog.message}</p>
                </div>
                <div className="flex justify-end border-t border-gray-200 bg-white px-5 py-3">
                  <button type="button" onClick={handleCloseSubmitFailureDialog} className="rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700">Close</button>
                </div>
      ...
      {/* Step 2: Select Access (JIT-specific) */}
      {currentStep === 2 && (
        <>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                {selectedUsers.map((user) => (
                  <div key={user.id} className="relative p-2 rounded-lg border-2 border-emerald-400 bg-white shadow-sm hover:shadow-md transition-all">
                    <button
          ...
          {selectedGroups.length > 0 && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-6">
              <h3 className="text-sm font-semibold mb-3 text-gray-900">Selected Groups ({selectedGroups.length})</h3>
              <div className="flex flex-wrap gap-2">
                {selectedGroups.map((g) => (
                  <span key={g.value} className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-50 text-blue-800 border border-blue-200" title={g.label}>
                    {g.label}
                  </span>
                ))}
              </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
            <JitSelectAccessTab
              ...
              onAppInstanceChange={(id) => setSelectedAppInstanceId(id || null)}
              showApplicationInstancesOnly={showApplicationInstancesOnly}
              onShowApplicationInstancesOnlyChange={(checked) => setShowApplicationInstancesOnly(checked)}
              onCatalogTypeChange={(value) => setCatalogTypeFilter(value)}
              onTagSearch={(tag) => setTagFilter(tag)}
            />
          </div>
        </>
      )}

      {/* Step 3: Details (JIT-specific) */}
      {currentStep === 3 && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <JitDetailsTab />
        </div>
      ...
      {showLeaveConfirm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true" aria-labelledby="leave-confirm-title">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <h2 id="leave-confirm-title" className="text-lg font-semibold text-gray-900 mb-2">Leave JIT Access?</h2>
            <p className="text-sm text-gray-600 mb-6">All data will be lost. Do you still want to leave?</p>
            <div className="flex justify-end gap-3">
              <button
                type="button"
                onClick={() => { setShowLeaveConfirm(false); pendingNavigateUrlRef.current = null; }}
